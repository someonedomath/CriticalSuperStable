...
%the continuous time multitype critical Galton-Watson processes.
...
%process known as $(\xi, \psi)$-superprocesses:
...
%is independent to the initial position $x$.
...
%does not depend the initial configuration $\mu$.
...
%Let $E$ be a locally compact separable metric space, the spatial motion 
...
%Using the argument in \cite{RenSongZhang2015Limit}, 
...
%It is also proved in \cite{RenSongZhang2015Limit} and \cite{RenSongZhang2017Central} 
...
%We refer to $\phi$ (resp. $\phi^*$) and $\lambda$ 
...
%end move
%For any $g_0, g_1\in \mathscr B^{++}({\mathbb R\times E})$, 
...
\begin{lem} \label{lem: asymptotic equivalent of integration}
	Suppose that 
%	$f_0,f_1\in \mathscr B^{++}_b({\mathbb R \times E})$ 
...
\begin{proof}
  ...
  \end{align}
	...
	%$h$ and $h_0$ are asymptotic equivalent at $\infty$.
...
\end{align}
%Therefore, integrating the both sides of \eqref{eq:mean-fkpp} with respect to measure 
...
%\cite{EckhoffKyprianouWinkel2015Spines}, \cite{EnglanderKyprianou2004Local} and \cite{LiuRenSong2009Llog} for example, 
...
%	If $F$ is a bounded Borel function on $E\times [0,1]\times [0,\infty)$ 
...
\begin{proof}
  ...
  %our superprocess $\{X;\mathbf P\}$ is non-persistent,
%  and more precisely,
that
  \begin{align}
...
%  the eigenfunction of semigroup $(P_t^\beta)_{t\geq 0}$, 
the principal eigenfunction of the semigroup $(P_t^\beta)_{t\geq 0}$, 
  ...
  %whose proof is postponed after the current proof.
...
%    Also from that $x \stackrel[x\to 0]{}{\sim} - \log(1-x)$, 
...
%and Proposition \ref{prop: convergence in a same speed}, we have
...
%The underline idea of the proof is to show that 
...
%the property of Poisson random measures, we have
...
%To complete the proof, we are left to verify the claim \eqref{eq:epsilon-2-one}.
...
\end{equation}
%The underlying idea of the proof is 
The main idea is
...
%For the rest of the proof, we use the following notation:
...
%new
\bibitem{KSV1} Kim, P., Song, R. and Vondracek, Z.:
...
\bibitem{KSV2} Kim, P., Song, R. and Vondracek, Z.:
