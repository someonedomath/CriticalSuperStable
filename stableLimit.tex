%%%----Versions-----------------------------
%stable-limit5.tex 2018/6/22  by
...
%    The study of the asymptotic behavior 
   The study of the asymptotic behaviors
    of critical branching particle systems has a long history.
...
%    and then  more appropriate scalings are needed.
   and thus more appropriate scalings are needed.
%Research in this direction was first gaven
Research in this direction was first conducted 
by Zolotarev \cite{Zolotarev1957More} in a simplified continuous time set-up,
	which is then extended by Slack \cite{Slack1968A-branching} to discrete time critical Galton-Watson processes allowing infinite 
%	variance:  Consider a critical Galton-Watson
	variance. The main result of  \cite{Slack1968A-branching} can be stated as follows. Consider a critical Galton-Watson
    process $\{(Z_n)_{n\geq 0}; P\}$
...
%	In order $\big\{ P(Z_n > 0) Z_n; P(\cdot | Z_n > 0)\big\}$ to have 
	In order for $\big\{ P(Z_n > 0) Z_n; P(\cdot | Z_n > 0)\big\}$ to have
	a non-degenerate weak limit, 
%	the offspring generating function must be of  
...
%	condition:
	...
	Let $\mathbf Z_n=(Z_n^{(1)}, \dots, Z_n^{(d)})$ be a critical, $d$-type, nonsingular and positively regular Galton-Watson process.
	 Denote by $\mathbf F(\mathbf s) = (\mathbf F_1(\mathbf s), \dots, \mathbf F_d(\mathbf s))$ 
%	 the corresponding offspring probability generating function,
     the generating function of the offspring distribution,
	and  by $\mathbf F^{(n)}(\mathbf s), ~ n>1,$ its $n$th iterates.
	Let $M$ be the mean matrix of $\mathbf Z$, $\mathbf v$ and $\mathbf u$ be 
%	its left and right principal  eigenvectors respectively, 
	its left and right  eigenvectors, respectively, 
	corresponding to the maximal eigenvalue 1, and normalized so that $\mathbf v \cdot \mathbf u = 1$ and $\mathbf 1 \cdot \mathbf u = 1$, with $\mathbf 1$ being the vector $(1,\dots, 1)$.
	Suppose that
...
\[ \label{eq: limit behavior of the exitinction probability without finite variance of multitype branching processes}
	...
	%(\mathbf i \cdot \mathbf u)^\alpha / \alpha,
...
\]
  where $\mathbf z^{(\alpha)}$ is a random variable with  Zolotarev's distribution with index  $\alpha$.
	For the converse of this problem, Vatutin \cite{Vatutin1977Limit} showed that in order for the left side of \eqref{eq: conditioned normalized multitype branching process} 
%	has a non-degenerate weak limit, 
   to have a non-degenerate weak limit, 
	one must have \eqref{eq: regularly varying condition for multitype branching process} for some $0 < \alpha \leq 1$.
...
\]
	...
	a class of critical superprocesses with general branching mechanism and general spatial 
	%motions:
	 motions. The main results of \cite{RenSongZhang2015Limit} are as follows.
...
\]
	...
	In order for the left side of \eqref{eq: conditional limit of CSBP} 
%	has non-trivial weak limit for
    to have a non-trivial weak limit for
...
\]
	...
	is regularly varying at infinity with index $\frac{1}{\gamma_0 - 1}$.
...
\]
	where $\eta_t := \big( C_X(\gamma_0 - 1) t \big)^{- \frac {1} {\gamma_0 - 1} },$ $C_X := \langle \mathbf 1_{\gamma(\cdot) = \gamma_0} \kappa \phi^{\gamma_0}, \phi^* \rangle_m$ and $\mathbf z^{(\gamma_0 - 1)}$ is a 
%	Zolotarev's random variable with index $\gamma_0 - 1$.
random variable with Zolotarev's distribution with index $\gamma_0 - 1$.
	Precise statements of the assumptions and the results are presented in the next
%		section.
    subsection.
    It is interesting to mention here that, even though the stable index $\gamma(x)$ is spatially dependent, the limiting behavior of the critical superprocess 
%    $(X_t)$   
   $\{X; \mathbf P\}$
...
%	be a $E$-valued Hunt process with its lifetime denoted by $\zeta$, 
	be an $E$-valued Hunt process with its lifetime denoted by $\zeta$, 
	and the branching mechanism $\psi$ be a function on $E\times[0,\infty)$ given by
	\eqref{eq: branching mechanism}.
%	We say a $\mathcal M^1_E$-valued Hunt process
	We say an $\mathcal M^1_E$-valued Hunt process
...
then $P_t:= P^0_t$ is the \emph{transition semigroup} of the process $\xi$.)
	...
	\]
%	This mean semigroup plays a central role in the study of the 
    The  mean semigroup plays a central role in the study of the 
...
%	This can be done under the following assumptions on the underlying motion
	This can be done under the following assumptions on the spatial motion
    $\xi$:
\begin{asp}{1}
\label{asp: 1}
...
Our second assumption on the spatial process 
    $\xi$
 is the following:
...
\[\label{eq:q(t,x,y)}
	...
	\quad t > 1.
\]
	We refer our readers to \cite{RenSongZhang2015Limit} for a list of examples of 
%	underlying processes satisfying Assumption \ref{asp: 1} and \ref{asp: 3}.
    processes satisfying Assumption \ref{asp: 1} and \ref{asp: 3}.
...
%	Under the Assumptions \ref{asp: 1} and \ref{asp: 3},
	Under Assumptions \ref{asp: 1} and \ref{asp: 3},
...
be a branching diffusion process, in a bounded domain,  with finite second moment.
...
$(t,x) \mapsto P_{\delta_x}(\|Y_t\| \neq 0)$ is tricky.
	...
	In this paper, we will  generalize these arguments to a class of 
%	critical superprocesses without finite second moment, 
    general critical superprocesses without finite second moment, 
	and establish Theorem \ref{thm: main theorem}(2).
...
%	Since in this paper, we do not assume the second moment condition, 
Since we do not assume the second moment condition in this paper,
	we can not use the method of double size-biased transform.
...
%	\cite{RenSongZhang2015Limit} (for critical superprocesses with finite variance), 
	\cite{RenSongZhang2015Limit} (for general critical superprocesses with finite variance), 
	the conditional weak convergence was proved in two steps.
...
\ref{sec: Ergodicity}, we discuss the ergodicity of the spine process.
    In Subsections
\ref{sec: proof of result 1} and \ref{sec: proof of result 2} we give the poofs of Theorem \ref{thm: main theorem}(1) and \ref{thm: main theorem}(2), respectively.
    In Subsection
...
%   an invertible, monotone and regularly varying function.
    a strictly monotone, regularly varying function.
%	In this paper, if a function $f$ is 	invertible,
	In this paper, if a function $f$ is strictly monotone, 
...
\]
	...
	Notice that, since $\alpha >0$, we must have $f$ is 
%	non-decreasing.
   strictly increasing.
...
%	which, by the non-decreasing nature of $f^{(-1)}$, gives that
	which, by the increasing nature of $f^{(-1)}$, gives that
...
%	\leq \lambda ^{\gamma_0}.
	\leq \lambda ^{\gamma_0}, 	\quad t\in (0,\infty).
\]
...
%	\geq \lambda ^{\gamma_0}.
	\geq \lambda ^{\gamma_0}, \quad t\in (0, \infty).
\]
...
\[
	...
	= \lambda ^{\gamma_0},	
%	\quad \lambda \in (0,1].
	\quad \lambda \in (0,1], t\in (0, \infty).
\]
...
%	= \lambda ^{\gamma_0}.
	= \lambda ^{\gamma_0}, \quad t\in (0, \infty).
\]
...
if $\mathbf P_{\delta_x}(\|X_t\|= 0) > 0$ for all $x\in E$ and $t> 0$.
...
\[ \label{eq: equation for N measure}
	...
	\quad \mu \in \mathcal M_E^1, t>0,
		f\in \mathscr B_b^+(E).
\]
	Taking $\mu = \delta_x$ for some $x\in E$, $f = \lambda \mathbf 1_E$ for $\lambda > 0$ in the above equation, 
%	and taking $\lambda \to \infty$, we can write
   and letting $\lambda \to \infty$, we get
...
\begin{itemize}
\item
	The \emph{spine process} $\{(\xi_t)_{0\leq t\leq T}; \dot{\mathbf P}^{(g,T)}_\mu\}$ is a copy of $\{(\xi_t)_{0\leq t\leq T}; \Pi^{(g,T)}_{\mu}\}$, where $\Pi^{(g,T)}_{\mu}$ is
	the $g(\xi_T) \exp\{-\int_0^T \beta(\xi_s)ds\}$-transform
	of the measure $\Pi_{\mu}(\cdot):=\int_{E}\mu(dx)\Pi_x(\cdot) $;
\item
%	Conditioned on $\{(\xi_t)_{0\leq t\leq T}; \dot{\mathbf P}^{(g,T)}_\mu\}$, 
Given $\{(\xi_t)_{0\leq t\leq T}; \dot{\mathbf P}^{(g,T)}_\mu\}$, 
%	\emph{the immigrate measure} 
the \emph{immigration measure} 
	$\{\mathbf n_T; \dot{\mathbf P}^{(g,T)}_\mu[\cdot |(\xi_t)_{0\leq t\leq T}]\}$ is a Poisson random measure on $[0,T] \times \mathbb W$
%	with density
	with intensity
\[
	\mathbf m^\xi_T(ds,dw)
...
%	is \emph{the spine representation of $\mathbb N^{(\phi)}_\mu$}  if:
	is a \emph{spine representation of $\mathbb N^{(\phi)}_\mu$}  if:
\begin{itemize}
\item
...
For each $x\in E$, let $\tilde \kappa(x) := \kappa(x) \mathbf 1_{\kappa(x)\geq \kappa_0} + \kappa_0 \mathbf 1_{\kappa(x) < \kappa_0}$ and $\tilde \gamma(x) := \gamma(x) \mathbf 1_{\gamma(x)\geq \gamma_0} + \gamma_0 \mathbf 1_{\gamma(x) < \gamma_0}$.
	...
	Define $\widetilde \psi(x,z) := - \beta(x)z+ \tilde \kappa(x)z^{\tilde \gamma(x)}$ for each $x\in E$ and $z\geq 0$, then we have that, for each $z\geq 0$, $\widetilde \psi(\cdot, z) = \psi(\cdot , z) $ 
...
\]
		...
		Notice that,
	under  Assumption \ref{asp: 1}, according to \eqref{eq: p-t-beta is comparable to phi phi-star}, $d\nu:= \phi^* dm$ defines a finite measure on $E$.
...
$v_t$ also satisfies the equation
...
\]
	Therefore, to prove Theorem \ref{thm: main theorem}(2), we only 
	%have to consider the speed of this convergence.
	 need to consider the speed of this convergence.
...
\label{prop: regularly varying of vt-phi-star}
	$\langle v_t,\phi^*\rangle_m $ is regularly varying at $\infty$ with index $-\frac{1}{\gamma_0-1}$.
	Furthermore, if $m(x: \gamma (x)= \gamma_0)>0$, then
\[
...
$\{(Y_t), (\xi_t),\mathbf n; \dot {\mathbf P}^{(\phi)}_\mu\}$
...
\]
	In fact, using the Markov property of the spine process $(\xi_t)$ and the property of Poisson random measures, we have
...
\begin{proof}[Proof of Proposition \ref{prop: regularly varying of vt-phi-star}]
	...
	Therefore, the map $t\mapsto \langle v_t,\phi^*  \rangle$ has an inverse on $(0,\infty)$
which is denoted by
\[
	R: (0,\infty) \to (0,\infty).
\]
...
for Lebesgue 
	Therefore, we have almost every $r\in (0,\infty)$.
\[\begin{split}
	s-t
	& = \int_t^s dr
...
We now check the regularly varying property of $R(r)$ at $r=0$.
	This can be done by considering the regularly varying property of 
	%$\big\langle \psi_0 \big(\cdot ,( 1 + \epsilon_{R(u)} ) u \phi \big), \phi^* \big\rangle_m$ at
%	$u = 0$.
 $u\to \big\langle \psi_0 \big(\cdot ,( 1 + \epsilon_{R(u)} ) u \phi \big), \phi^* \big\rangle_m$
 at 0.
...
$\kappa(x)\phi(x)^\gamma(x)$ is bounded and the measure $\phi^* dm$ is finite, we have that $\langle \psi_0\big(\cdot,(1+\epsilon_{R(u)})u\phi \big), \phi^* \rangle_m$ is regularly varying at $u = 0$ with index $\gamma_0$.
...
\[\begin{split}
	...
	=: C_X u^{\gamma_0}.
\end{split}\]
	Therefore, we have $\big\langle \psi_0 \big(x,( 1 + \epsilon_{R(u)} ) u \phi \big), \phi^* \big\rangle_m^{-1} = u^{-\gamma_0} l(u)$ where $l(u)$ converges to the constant $C_X^{-1}$ when $u \to 0$.
	Now according to
...
\label{sec: conditional distribution}
    ...
    be a $(\xi, \psi)$-superprocess satisfying
	Assumptions \ref{asp: 1}--\ref{asp: 4}.
	...
	converges weakly
	to a non-degenerate distribution for a large class of functions $f$ and initial configurations $\mu$.
...
the desired result that, for any $f\in \mathscr B^+_b(E)$ and $x\in E$,
 $\big\{\big(\eta_t X_t(f)\big)_{t \geq 0}; \mathbf P_{\delta_x}(\cdot | X_t \not \equiv 0)\big\}$
 converge weakly
...
\]
	...
	In order to investigate the convergence of $(\frac{ V_t(\theta \eta_t f)(x)}{\eta_t})$, 
	%we need to characterize the dynamics of $V_t(\theta f)$ as $\theta$ is varying.
	we need to investigate the properties of $\theta\to V_t(\theta f)$.
	(Note that \eqref{eq:mean-fkpp} 
	%only characterizes the dynamics of $V_t(\theta f)$ as $t$ is varying.)
	only gives the the dynamics of $t\to V_t(\theta f)$.)
...
%	$\mathbf m^\xi_T$.
where $\{(\xi)_{0\le t\le T}, \mathbf n_T, (Y)_{0\le t\le T}; \dot {\mathbf P}^{(f,T)}_x\}$ is a spine representation of $\mathbb N^{W_T(f)}_x$ with $\mathbf m^\xi_T$ being the 
intensity if the immigration measure $\mathbf n_T$ conditioned on $\{(\xi)_{0\le t\le T}; \dot {\mathbf P}^{(f,T)}_x\}$.
	From this, we have
...
%	the Minkowski inequality of the $L^p$ norm with $p = \frac{1}{\gamma_0 - 1} > 1$, we have
by the $L^p$ Minkowski inequality with $p = \frac{1}{\gamma_0 - 1} > 1$, we have
\[\begin{split}
...
\label{sec: proof of result 3}
	...
	As have been discussed in Subsection \ref{sec: conditional distribution},
...
\]
	...
	Finally, 
	%recalling the definition of $M$, by showing that $M\equiv 0$, the proof is complete.
	by the definition of $M$,  $M\equiv 0$ implies the desired assertion.
