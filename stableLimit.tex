%!TEX spellcheck
%%%----Versions-----------------------------
%stable-limit7.tex 2018/6/29 by Renming-----
...
\begin{abstract}
	In this paper, we consider a critical $(\xi,\psi)$-superprocess
$\{X; \mathbf P_\mu\}$.
	$\xi$, the spatial motion of the superprocess, is a
%Hunt process, on a locally compact separable metric space $E$,
%	satisfying an intrinsically ultracontractive condition.
Hunt process on a locally compact separable metric space $E$ with reference measure $m$.
...
\begin{equation}
	...
	\quad x\in E, z \geq 0,
\end{equation}
  with $\beta \in \mathscr B_b(E),$ $\gamma \in \mathscr B^+_b(E)$ and $\kappa \in \mathscr B^+_b(E)$ satisfying
\begin{equation}
	1
	< \gamma(\cdot )
	<2;
...
%	We show that 
	...
	Furthermore, if $m(x: \gamma(x) = \gamma_0)>0$, 
%	we show that
 we show that, for any finite initial measure $\mu$ on $E$, 
\begin{equation}
%	\mathbf P_{\delta_x}( \|X_t\| \neq0)
%	\stackrel[t\to \infty]{}{\sim} \phi(x) \eta_t,
\mathbf P_{\mu}( \|X_t\| \neq0)
	\stackrel[t\to \infty]{}{\sim} \langle\phi \eta_t,
\end{equation}
...
\end{equation}
%	where 
 where $\phi$ ($\phi^*$, resp.) is the principal eigenfunction of the mean semigroup 
 (the dual of of the eman semigroup) of the superprocess, 	
	$\eta_t := \big( C_X(\gamma_0 - 1) t \big)^{- \frac {1} {\gamma_0 - 1} },$ $C_X := \langle \mathbf 1_{\gamma(\cdot) = \gamma_0} \kappa \phi^{\gamma_0}, \phi^* \rangle_m$ and $\mathbf z^{(\gamma_0 - 1)}$ is a random variable with Laplace transform
\begin{equation}
...
\subjclass[2010]{Premary: 60J68; 60F05. Secondary 60J80; 60J25}
...
\end{equation}
	...
	critical branching processes with finite 2nd moment, see \cite{AsmussenHering1983Branching}, \cite{AthreyaNey1974Functionals}, \cite{AthreyaNey1972Branching} and \cite{JoffeSpitzer1967On-multitype} for example.
...
%\begin{equation}
\begin{equation}\label{e:zloltarev}
	E[e^{- u \mathbf z^{(\alpha)}}]
	= 1 - (1+ u^{-\alpha})^{-1/\alpha},
	\quad u \geq 0.
\end{equation}
...
\end{equation}
	where $\mathbf z^{(\alpha)}$ is a random variable with  
%	Zolotarev's distribution with index  $\alpha$.
   Laplace transform given by \eqref{e:zloltarev}.
...
\end{equation}
    where $\mathbf z^{(\gamma - 1)}$ is a random variable with 
%    Zolotarev's distribution with index $\gamma - 1$.
     Laplace transform given by \eqref{e:zloltarev} (with $\alpha=\gamma-1$).
...
\end{equation}
	where $\eta_t := \big( C_X(\gamma_0 - 1) t \big)^{- \frac {1} {\gamma_0 - 1} },$ $C_X := \langle \mathbf 1_{\gamma(\cdot) = \gamma_0} \kappa \phi^{\gamma_0}, \phi^* \rangle_m$ and $\mathbf z^{(\gamma_0 - 1)}$ is a random variable with 
%	Zolotarev's distribution with index $\gamma_0 - 1$.
   Laplace transform given by \eqref{e:zloltarev} (with $\alpha=\gamma-1$).
...
\subsection{Model and results}
    ...
    We also write $\langle f, g\rangle_m$ for $\int_E fg dm$ to emphasize that it is the inner product in the Hilbert space
    %$L^2(m)$.
    $L^2(E, m)$.
...
\end{equation}
\end{itemize}
	Here, $\eta_t := \big( C_X(\gamma_0 - 1) t \big)^{- \frac {1} {\gamma_0 - 1} }$, $C_X := \langle \mathbf 1_{\gamma(\cdot) = \gamma_0} \kappa \phi^{\gamma_0}, \phi^* \rangle_m$ and $\mathbf z^{(\gamma_0 - 1)}$ is a random variable with 
%	Zolotarev's distribution with index $\gamma_0 - 1$.
   Laplace transform given by \eqref{e:zloltarev} (with $\alpha=\gamma-1$).
\end{thm}
...
\subsection{Methods and overview}
 ...
 general critical superprocesses with finite second moment.
	...
	For the conditional weak convergence result, i.e., Theorem \ref{thm: main theorem}(3), we use a fact that 
%	the Laplace transform of Zolotarev's distribution can be characterized by a non-linear 
the Laplace transform given in \eqref{e:zloltarev} can be characterized by a non-linear 
	...
	\cite{RenSongSun2017A-2-spine, RenSongSun2017Spine}.
    ...
    In Subsection \ref{sec: Characterizing the Zolotarev's distribution using an non-linear delay equation}, we give the equation that characterize 
%    Zolotarev's distribution.
   the distribution with Laplace transform \eqref{e:zloltarev}.
...
\end{split}\end{equation}
	Now since $r\mapsto \langle v_r,\phi^*\rangle_m$ is the inverse of $r\mapsto R(r)$, from Corollary \ref{cro: regularly varing and inverse with alpha < 0} and the above,
	we have
\begin{equation}
...
\begin{thebibliography}{10}
	...
	\emph{A 2-spine decomposition of the critical 
% galton-watson tree and a probabilistic proof of yaglom's theorem.}
Galton-Watson tree and a probabilistic proof of Yaglom's theorem.}
	\ARXIV{1706.07125}
