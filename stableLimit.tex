...
\subsection{Background}
	...
	Assuming that 
	%the offspring generating function $f(s)$ 
...
\]
	...
	%the matrix $E(\mathbf s)$ is defined by
...
\]
    Let 
    %$a_n := \mathbf v \cdot (\mathbf 1 - \mathbf F_n(\mathbf 0))$, 
...
%deleted	
	%(see \cite[Section 6.3]{AsmussenHering1983Branching} and \cite[Section 6.4]{AsmussenHering1983Branching}).
%end deleted
...
\]
	...
	A simpler
	version of this question has already been answered in the context of continuous-state branching processes %(CSBP) 
	(CSBPs)
...
\]
	...
	%some positive function $(\lambda_t)$,
...
\]
	...
	%is regularly varying with index $\frac{1}{\gamma_0 - 1}$ at $\infty$..
...
\subsection{Model and results}
   ...
   $\xi:=\{(\xi_t)_{t\geq 0};(\Pi_x)_{x\in E}\}$ 
	...
	%$\{(X_t); (\mathbf P_\mu)_{\mu \in \mathcal M^1_E}\}$ 
...
then $P_t:= P^0_t$ is the \emph{transition semigroup} of the process $\xi$.)
	...
	%or \eqref{eq: Ygalom type result without 2rd moment}, 
	or \eqref{eq: result2},
...
\]
	...
	%$(P^\beta_t)_{t \geq 0}$ and its dual semigroup $(P^{\beta *}_t)_{t \geq 0}$ 
...
%deleted    
    %$(e^{-\lambda t}p^\beta_t(x,y))_{x,y\in E}$ is,  uniformly in $x$ and $y$, asymptotically equivalent to $\big(\phi(x)\phi^*(y)\big)_{x,y\in E}$ as $t\to \infty$ in the sense that,
%deleted
...
\begin{itemize}
	...
	%converges to $0$ as $t\to \infty$, and is regularly varying at $\infty$ with index $-\frac{1}{\gamma_0-1}$.
...
\subsection{Methods and overview}
	...
	%In this paper, to establish Theorem \ref{thm: main theorem}(2) and Theorem \ref{thm: main theorem}(3), we use a spine decomposition of  $\{X; \mathbf P\}$.
	...
	%the one-dimensional conditioned distributions 
	...
	%using an $x^2$-transform distribution equation; 
	...
	However, the 
	%$x^2$-transform of a random variable requires the existence of the second moment.
    ...
    %a general functions $f$.
    more general testing functions.
	%Since we are not assuming the second moment condition, this type of argument does not work in the present case.
	...
	%all general functions $f$ at the same time.
...
%deleted
	...
	%The transformed process behaves similarly as in the classical case, despite the fact that the spine processes $\{(\xi_t)_{0\leq t\leq T}; \Pi^{(g,T)}\}$ are not consistent for all $T>0$.
%end deleted
...
%ZY: I agree. Let us only give the monotone version.
...
\]
	...
	It is easy to verify that 
	%$g$ is also invertible with $g^{(-1)}(t) := f^{(-1)}(t)^{-1}$.
...
\]
	...
	%Now, further assume that $f$ is invertible and monotone, and 
	Now, assume that
...
%\subsubsection{Non-presistency}
	...
	%\emph{canonical space of $X$}.
	\emph{canonical space of $(X_t)$}.
	%In fact, $X$ can be viewed 
...
%deleted    
    %The we see that $\{(X_t(\phi))_{t\geq 0};\mathbf P_{\mu}\}$ is nonnegative martingale,  and $\{(W_t(\phi))_{t\geq 0};\mathbb N_{\mu}\}$ is also a nonnegative martingale.
%end delted
...
\item
	...
	%the $g(\xi_T) e^{-\int_0^T \beta(\xi_s)ds}$-transform 
...
\end{cro}
	...
	%Remark \ref{rem: initial configuration of transformed process}
...
\]
	...
	%According to \eqref{eq:mean-fkpp}, 
...
\begin{proof}[Proof of Proposition \ref{prop: convergence in a same speed}]
	...
	%$\{(Y_t)_{t\geq 0}, (\xi_t)_{t\geq 0},\mathbf m^\xi, \mathbf n; \dot {\mathbf P}^{(\phi)}_\mu\}$ 
...
\end{split}\]
	...
	%Using an argument similar to that used in the proof of \cite[Lemma 5.3]{RenSongSun2017Spine}, we can get that, if $t_0>1$,
	We claim that
...
\end{split}\]
	...
	%according to \eqref{eq: starting point of phi-1v_t(x)} and \eqref{eq: some equations for PY-1}, we have
...
\[\label{vts-inequality}\begin{split}
	...
	%\cdot \|\phi^{-1}v_{t-s}\|^{\gamma_0-1}_\infty (1+c_0 e^{-c_1 t_0}).
	\cdot \|\phi^{-1}v_{t-t_0}\|^{\gamma_0-1}_\infty (1+c_0 e^{-c_1 t_0}).
\end{split}\]
	%Since $\phi^{-1}v_s$ converges to $0$ uniformly as $s\to \infty$, 
...
\begin{proof}[Proof of Proposition \ref{prop: regularly varying of vt-phi-star}]
	...
	On the other hand, 
	%since $\phi^{-1}v_t$ converges to $0$ uniformly in $E$, 
	according to \eqref{eq: uniform converges to 0},
...
\]
	...
	%$0$.
	$u = 0$.
    %In fact, according to Proposition \ref{prop: convergence in a same speed},  $1+ \epsilon_{R(u)}(x) \stackrel[u\to 0]{x\in E}{\sim} 1$. 
...
\end{split}\]
	...
	Now according to 
	%Corollary \ref{cro: power law and ingetration} 
...
\end{split}\]
	...
	%\eqref{eq: inverse and power equivalent with alpha < 0},
...
\begin{prop}
	%For any $f\in pb\mathscr B^+_b(E),\theta \geq 0,x\in E$ and $T>0$, we 
...
\]
	...
	%then by Campbell's formula and \eqref{eq: definition of Gamma function}, we have
...
\end{split}\]
	...
	%we have that $G(\theta) \leq c\theta$ and $G_0(\theta) \leq c \theta$.
...
\]
	...
	Step 1: 
	%We will give some upper bounds for $G,g, J_G, J'_G, J_g$ and $J'_g$ respectively.
...
\end{split}\]
	...
	Notice that, 
	%by \eqref{eq: asymptotic for q_t(x,y)},
	by \eqref{eq:q(t,x,y)}
\[
