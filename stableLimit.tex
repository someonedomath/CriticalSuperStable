%!TEX spellcheck
%%%----Versions-----------------------------
%stable-limit6.tex 2018/6/27 by Zhenyao-----
%stable-limit5.tex 2018/6/22 by Renming------
...
%\usepackage{amsmath}
%end Deleted 
%ZY: \mathtoolsset{showonlyrefs} commond can do the trick of 'autonum' package.
%ZY: amsmath package is included in the amsart document class
...
\begin{abstract}
%new
	In this paper, we consider a critical $(\xi,\psi)$-superprocess $\{(X_t)_{t\geq 0}; (\mathbf P_\mu)_{\mu \in \mathcal M^1_E}\}$.
	$\xi$, the spatial motion of the superprocess, is a Hunt process on a locally compact separable metric space $E$, satisfying an intrinsically ultracontractive condition.
...
\begin{equation}
	...
	\quad x\in E, z \geq 0,
\end{equation}
	where $\beta \in \mathscr B_b(E),$ $\gamma \in \mathscr B^+_b(E)$ and $\kappa \in \mathscr B^+_b(E)$ satisfy 
\begin{equation}
	1 
	< \gamma(\cdot )
	<2; 
...
\end{equation} 
	where $\eta_t := \big( C_X(\gamma_0 - 1) t \big)^{- \frac {1} {\gamma_0 - 1} },$ $C_X := \langle \mathbf 1_{\gamma(\cdot) = \gamma_0} \kappa \phi^{\gamma_0}, \phi^* \rangle_m$ and $\mathbf z^{(\gamma_0 - 1)}$ is a random variable with Laplace transform
\begin{equation}
...
\end{equation}
	...
	%Suppse that $(Z_t)$ is a CSBP with ciritical branching mechanism $\psi$ satisfying Grey's condition.
...
\end{equation}
	...
	%We will show that $\mathbf P_{\delta_x}( X_t \not \equiv 0) $ converges to $0$ as $t\to \infty$ and is regularly varying at infinity with index $\frac{1}{\gamma_0 - 1}$.
...
\begin{itemize}
\item[(1)] 
	%$\{X; \mathbf P\}$ is non-persistent, that is, for each $t > 0$ and $x\in E$, $\mathbf P_{\delta_x}( X_t \equiv 0) > 0$.
	$\{X; \mathbf P\}$ is non-persistent, that is, for each $t > 0$ and $x\in E$, $\mathbf P_{\delta_x}( \| X_t\| \neq 0) > 0$.
\item[(2)] 
	%For each $x\in E$, $\mathbf P_{\delta_x}(X_t \not \equiv 0)$ converges to $0$, and is regularly varying with index $-\frac{1}{\gamma_0-1}$ as $t\to \infty$.
...
%deleted
	%The following lemma concerns the inverse of a strictly monotone, regularly varying function.
...
%\label{lem: regularly variation and inverse}
	%Suppose that $f$ is a strictly monotone map from $(0,\infty)$ to $(0,\infty)$.
	%Then, for each $\alpha > 0$, we have
%\begin{equation} \label{eq: inverse of a regularly varying function at infinity with alpha > 0}
	%f
...
%\end{equation}
	%and
%\begin{equation} \label{eq: inverse of a regularly varying function at 0 with alpha > 0}
	%f
...
%\begin{equation} \label{eq: inverse of a regularly varying function with alpha < 0}
	%f
	%\in \mathcal R^\infty_{\alpha}
	%\iff f^{(-1)}
	%\in \mathcal R^0_{1/\alpha}.
...
%\begin{equation}\label{eq: inverse and power equivalent at infinity with alpha > 0}
	%f(t)
...
%\end{equation}
	%and
%\begin{equation}\label{eq: inverse and power equivalent at 0 with alpha > 0}
	%f(t)
...
%\begin{equation}\label{eq: inverse and power equivalent with alpha < 0}
	%f(t)
	%\stackrel[t\to \infty]{}{\sim} c t^\alpha
	%\iff f^{(-1)}(t)
	%\stackrel[t\to 0]{}{\sim} c^{-1/\alpha} t^{1/\alpha}.
...
%If $\alpha>0$, then according to \cite[Theorem 1.5.12.]{BinghamGoldieTeugels1989Regular}, we have \eqref{eq: inverse of a regularly varying function at infinity with alpha > 0} is true.
	...
	%It is  easy to verify that $h$ is strictly monotone with $h^{(-1)}(t) := f^{(-1)}(t^{-1})^{-1}$.
	%Therefore,
%\begin{equation}
	%f
	%\in \mathcal R^0_{\alpha}
...
%\end{equation}
	...
	%It is easy to verify that $g$ is strictly monotone with $g^{(-1)}(t) := f^{(-1)}(t)^{-1}$.
	%Therefore,
%\begin{equation}
	%f
	%\in \mathcal R^\infty_{\alpha}
...
%\end{equation}
	...
	%Now, assume that $f(t) \stackrel[t\to \infty]{}{\sim} c t^\alpha$ for some $\alpha > 0$ and $c > 0$, we want to show that $f^{(-1)}(t) \stackrel[t\to \infty]{}{\sim} c^{-1/\alpha} t^{1/\alpha}$.
	%Notice that, since $\alpha >0$, we must have $f$ is strictly increasing.
...
%\begin{equation}\label{eq: inverse and inequality}
...
\item
	...
	%with mean measure $\mathbb N_\mu(\cdot):= \int_E \mathbb N_x(\cdot )\mu(dx)$,
...
\end{equation}
	%Taking $\mu = \delta_x$ for some $x\in E$, $f = \lambda \mathbf 1_E$ for $\lambda > 0$ in the above equation, and letting $\lambda \to \infty$, we get
...
%end new
	%We denote  the $W_T(g)$-transform of $\mathbb N_\mu$ and the $X_T(g)$-transform of $\mathbf P_\mu$ by $\mathbb N^{W_T(g)}_\mu$ and $\mathbf P_\mu^{X_T(g)}$ respectively.
...
\end{equation}
	...
	%There is no need to restrict the corresponding processes on the finite interval $[0,T]$.
...
\label{sec: Ergodicity}
	%In this subsection, we discuss the ergodicity of the spine processes $\{(\xi_t); (\Pi^{(\phi)}_x)\}$ under Assumptions \ref{asp: 1}--\ref{asp: 3}.
...
\end{equation}
	...
	%we see that $v_t$ satisfies the equation
...
%deleted
	%Notice that Theorem \ref{thm: main theorem}(2) is simply a corollary of Propositions \ref{prop: asymptotic equivalence of vtphi} and \ref{prop: regularly varying of vt-phi-star}.
%end delted
...
\end{split}\end{equation}
	...
	%using the fact that that $v_t$ is non-increasing in $t$, we get
...
\end{split}\end{equation}
	%Now since $r\mapsto \langle v_r,\phi^*\rangle_m$ is the inverse of $r\mapsto R(r)$, from \eqref{eq: inverse and power equivalent with alpha < 0} and above,
...
\end{equation}
 ...
 \end{equation}
	...
	%From the point of view of Laplace transforms, the desired result that, for any $f\in \mathscr B^+_b(E)$ and $x\in E$, $\big\{\big(\eta_t X_t(f)\big)_{t \geq 0}; \mathbf P_{\delta_x}(\cdot | X_t \not \equiv 0)\big\}$ converge weakly to some probability distribution $F_{f,x}$ is equivalent to the following convergence:
...
\end{equation}
	%Therefore, to establish the weak convergence of $\big\{\big(\eta_t X_t(f)\big)_{t \geq 0}; \mathbf P_{\delta_x}(\cdot | \|X_t\|\neq 0)\big\}$, one only needs to verify \eqref{eq: equivalent result}.
...
\label{sec: proof of result 3}
	...
	%As have been discussed in Subsection \ref{sec: conditional distribution}, in order to prove Theorem \ref{thm: main theorem}(3), we only need to show that
...
\end{equation}
	...
	%Finally, by the definition of $M$,  $M\equiv 0$ implies the desired assertion.
	Finally, by the definition of $M$,  $M\equiv 0$ implies the desired assertion \eqref{eq: we only need to proof this}.

%ZY: I changed the format of the bibliography without marks 
