...
% Consider a critical Galton-Watson process $\{(Z_n)_{n\geq 0}; P\}$ with infinite variance.
...
\begin{lem} \label{lem: asymptotic equivalent of integration}
  ...
  % If $m$ is a finite measure on $(E, \mathscr E)$, then
  If $\rho$ is a finite non-degenerate measure on $(E, \mathscr E)$, then
  % Sun: I changed several m into \rho without further mentioning
  \begin{align}
...
\begin{lem}\label{lem:regularly_variation_and_integration}
  % Let $(E, \mathscr E)$ be a measurable space and $m$ a  non-degenerate measure on $(E, \mathscr E)$.
...
% Sun: I changed several gamma into alpha without further mentioning. 
% Sun: I also changed m into \rho in this lemma.
  \begin{align}
    \alpha_0
    := \operatorname*{ess\,inf}_{\rho(dx)} \alpha(x)
    % := \sup\{r:\rho(x:\alpha(x) < r) = 0\}.
...
\begin{proof}
  ...
  % SUN: I changed several \lambda to u without further mentioning.
...
% the measure-valued process $(W_t)_{0\leq t\leq T}$ can be decomposed as an measure-valued immigration process along the trajectory of the spine process in a Poissonian way.
...
\begin{proof}
  ...
  % The main idea is to show that both $\epsilon_x^1(t_0,t)$ and $\epsilon_x^2(t_0,t)$ are very small compared to $\dot {\mathbf P}_\nu^{(\phi)}[Y_t^{(t_0,t]}(\phi)^{-1}]$ provided $t_0$ and $t- t_0$ are large enough.
...
% Using the Markov property of the spine process and properties Poisson random measures, we have
